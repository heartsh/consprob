...
fn main() {
  ...
  let gap_num = if opts.opt_present("gap_num") {
    opts.opt_str("gap_num").expect("Failed to get a gap number from command arguments.").parse().expect("Failed to parse a gap number.")
  } else {
    DEFAULT_GAP_NUM
  };
  ...
  thread_pool.scoped(|scope| {
    for (lbpp_mat, lnbpp_mat, fasta_record) in multizip((lbpp_mats.iter_mut(), lnbpp_mats.iter_mut(), fasta_records.iter())) {
  ...
  thread_pool.scoped(|scope| {
    for (rna_id_pair, sta_fe_params) in sta_fe_param_sets_with_rna_id_pairs.iter_mut() {
      ...
      scope.execute(move || {
        *sta_fe_params = StaFeParams::new(rna_id_pair, ref_2_fasta_records, gap_num, ref_2_lbpp_mats, ref_2_lnbpp_mats);
      });
    }
  });
  ...
  let output_file_header = format!(" in this file = \"{}\".\n; The values of the parameters used to compute these matrices are as follows.\n; \"min_base_pair_prob\" = {}, \"gap_num\" = {}, \"max_base_pairing_span\" = {}, \"num_of_times_of_expect_max_iter\" = {}, \"num_of_threads\" = {}.", input_file_path.display(), min_bpp, gap_num, max_bp_span, num_of_times_of_expect_max_iter, num_of_threads);
