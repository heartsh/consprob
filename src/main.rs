...
const DEFAULT_STA_FE_SCALE_PARAM: LogProb = 0.1;
const DEFAULT_MIN_BPP: Prob = 0.1;
...
fn main() {
  ...
  for _ in 0 .. num_of_times_of_expect_max_iter {
    thread_pool.scoped(|scope| {
      for (rna_id_pair, lstapmt) in lstapmts_with_rna_id_pairs.iter_mut() {
        ...
        scope.execute(move || {
          *lstapmt = io_algo_4_rna_lstapmt(&seq_len_pair, sta_fe_params, sta_fe_scale_param, max_gap_num);
        });
      }
    });
    thread_pool.scoped(|scope| {
  ...
  let output_file_header = format!(" in this file = \"{}\".\n; The values of the parameters used to compute these matrices are as follows.\n; \"struct_align_free_energy_scale_param\" = {}, \"min_base_pair_prob\" = {}, \"gap_num\" = {}, \"max_base_pairing_span\" = {}, \"num_of_times_of_expect_max_iter\" = {}, \"num_of_threads\" = {}.", input_file_path.display(), sta_fe_scale_param, min_bpp, gap_num, max_bp_span, num_of_times_of_expect_max_iter, num_of_threads);
